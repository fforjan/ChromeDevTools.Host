...
namespace ChromeDevTools.Host.FwkConsoleApp
{
    public class WebSockerWrapper : IWebSocket
    {
        ...
        public async Task<Receiveinfo> ReceiveAsync(CancellationToken token)
        {
            ...
            var request = await this.WebSocket.ReceiveAsync(new ArraySegment<byte>(buffer), CancellationToken.None);
            ...
            else
            {
                ...
                {
                    IsClosed = false,
                    Content = Encoding.ASCII.GetString(buffer, 0, request.Count)
                };
            }
        }
    }
