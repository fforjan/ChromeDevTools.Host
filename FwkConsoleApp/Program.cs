    ...
    class Program
    {
        static async Task Main()
        {
            ...
            while (true)
            {
                ...
                switch (i % 4)
                {
                    case 0: await ChromeSessionWebServer.ForEach(_ => _.RuntimeHandle.Log($"Ticks : <message> {i}")); break;
                    case 1: await ChromeSessionWebServer.ForEach(_ => _.RuntimeHandle.Warning($"Ticks : <warning> {i}")); break;
                    case 2: await ChromeSessionWebServer.ForEach(_ => _.RuntimeHandle.Error($"Ticks : <error> {i}"));break;
                    case 3: await ChromeSessionWebServer.ForEach(_ => _.RuntimeHandle.Debug($"Ticks : <debug> {i}")); break;
                }
